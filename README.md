...
- 精确改写首页、Hermes 资源与结算页三个美团单车 JSON 接口。
- 删除腾讯微信广告及非 `MOBIKE` 广告策略项。
- 支持结算页 `adsResource[].infos` 的嵌套广告分组。
- 确认广告后同时移除已空的 `adsResource` 父容器，避免黑色弹窗或空白占位残留。
...
- 结算页规则先删除 `adsResource` 中有腾讯微信、`ADS_STRATEGY` 或微信 `adunit-` 组合强标记的对象，再删除因此变空的广告容器；不会删除会员续卡引导或其他结算页内容。
